...
class Password:
    ...
    def respond(self) -> AuthResponse:
        ...
        if not user:
            raise InvalidCredentials(message='User not exist')
        password = self.request_data.password
        # 用户密码校验
        if not (user.password and PwdService.verify_password(password, user.password)):
            raise InvalidCredentials(message='Incorrect email or password')

        # 用户状态校验
        if not user.is_enabled():
            raise InvalidCredentials(message='Inactive user')
...
class Refresh:
    ...
    def respond(self) -> RefreshResponse:
        if not self.request_data.access_token:
            raise InvalidToken(message='未知的令牌')
